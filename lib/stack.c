...
stack_init(Stack *s, int nmemb, int realloccount, long size)
{
	...
	/*added after tests with nhcustom2, why were they not set from the start???? how did it work without doing this?????*/
	s->stacki = 0;
	s->count = 0;
...
stack_getslot(Stack *s, int *i)
{
	...
	for(*i = 0; *i < s->max; *i+=1)
		if(!s->used[*i])
			return (char *)s->stack + *i * s->slotsize;
	return NULL;
}
...
/*similar to stack_get(), but it will return a valid slot
 *no matter the size of the stack, and will realloc memory
 *to get to the index if necessary. Use with care.
 */
void *
stack_get_force(Stack *s, int i)
{
	if(i < 0)
		return NULL;

	if(i >= s->max)
		if(stack_realloc(s, s->max +  i / s->realloccount + 1) < 0) /*+1 to avoid rounding issues*/
			return NULL;
...
int
stack_getmax(Stack *s)
{
	return s->max;
}

int
stack_getcount(Stack *s)
{
	return s->count;
}
...
stack_getnextused(Stack *s, int *i)
{
	...
	*i+=1;
	if(ok)
		return (char *)s->stack + (*i-1) * s->slotsize;
	return NULL;
}
...
stack_add(Stack *s, void *p)
{
	...
	if(s->count >= s->max){
		stack_realloc(s, s->max + s->realloccount);
		i = s->count; /*modified after tests with nhcustom2*/
	}
	for(; i < s->max; i++)
		if(s->used[i] == 0)
...
stack_addtoslot(Stack *s, void *p, int id)
{
	...
	memcpy((char *)s->stack + id * s->slotsize, p, s->slotsize);
	s->used[id] = 1;
...
int
stack_memset(Stack *s, unsigned int offset, int c, unsigned int size)
{
	unsigned int maxb = s->max * s->slotsize;

	/*checks to not overflow*/
	if(offset > maxb)
		return -1;
	...
	memset((char *)s->stack + offset, c, size);
	return 0;
}
